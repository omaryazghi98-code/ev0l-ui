...
type GooeyItem = { label: string; href: string }
...
type GooeyNavProps = {
  ...
  activeIndex?: number
  onItemClick?: (item: GooeyItem, index: number) => void
  className?: string
}
...
const GooeyNav = ({
  ...
  const activate = (index: number, element: Element) => {
          ...
          {items.map((item, index) => (
            <li key={item.href} className={currentActiveIndex === index ? 'active' : ''}>
              <a
                href={item.href}
                onClick={(event) => {
                  if (onItemClick) event.preventDefault()
                  activate(index, event.currentTarget.parentElement ?? event.currentTarget)
                }}
              >
                {item.label}
              </a>
